...
func (a App) GetLink(ctx context.Context, key string) (*Link, errs.Error) {
	...
	if err != nil {
		return nil, errs.E(ctx, errs.SeverityCritical, errs.KindInternal, fmt.Errorf("decoding key [%s] failed: %w", key, err))
	}
...
func (a App) DeleteLink(ctx context.Context, key string) errs.Error {
	...
	id, err := a.tokenizer.Decode(key)
	if err != nil {
		return errs.E(ctx, errs.SeverityCritical, errs.KindInternal, fmt.Errorf("decoding key [%s] failed: %w", key, err))
	}
